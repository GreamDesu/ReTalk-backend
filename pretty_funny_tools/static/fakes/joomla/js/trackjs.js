...
return oldValue+separator+newValue;}};PostAffCookieManager=function(){var flash=null,flashVersion=null;var visitorCookie=new PostAffCookie('PAPVisitorId');var _doNotUseFlashCookie=false;var oldCookies=new PostAssoc();addOldCookie(new PostAffCookie('PAPCookie_Sale'));addOldCookie(new PostAffCookie('PAPCookie_FirstClick'));addOldCookie(new PostAffCookie('PAPCookie_LastClick'));function addOldCookie(cookie){oldCookies[cookie.name]=cookie;}
...
document.cookie=name+'='+encodeURIComponent(value)+';expires='+expired.toGMTString()+';path=/';};PostAffCookie.deleteHttpCookie=function(name){expired=new Date(0);PostAffCookie.setHttpCookie(name,'',expired);};PostAffRequest=function(cookieManager){this.cookieManager=cookieManager;this.sendCalled=false;this.send=function(){this.sendCalled=true;this.loadHttpCookies();if(this.cookieManager.isFlashActive()){this.cookieManager.readAllFlashCookies();var self=this;setTimeout(function(){self.callTrackScript();},1000);return;}
